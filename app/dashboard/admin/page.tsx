...
export default function AdminPage() {
  ...
  const roleLabel: Record<string, string> = {
    ...
    cliente_admin: 'Cliente Admin', cliente_operacional: 'Operacional', cliente_visualizacao: 'Visualização',
  }
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1100px' }}>
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '24px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '4px', width: 'fit-content' }}>
        {(['clients', 'users'] as const).map(t => (
          <button
            ...
            onClick={() => setTab(t)}
            style={{
              padding: '8px 20px', borderRadius: '7px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', border: 'none',
      ...
      {loading ? (
          ...
          <div className="card" style={{ overflow: 'hidden' }}>
            {clients.length === 0 ? (
              ...
              <div key={c.id} style={{
                ...
                padding: '14px 18px', borderBottom: i < clients.length - 1 ? '1px solid var(--border)' : 'none',
              }}>
                <div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '16px' }}>
            {isAdmin && <button className="btn-primary" onClick={() => setShowNewUser(true)}>+ Novo Usuário</button>}
          </div>
          <div className="card" style={{ overflow: 'hidden' }}>
            {users.length === 0 ? (
              ...
              return (
                <div key={u.id} style={{
                  display: 'grid', gridTemplateColumns: '1fr auto auto auto',
                  alignItems: 'center', gap: '16px',
                  padding: '14px 18px', borderBottom: i < users.length - 1 ? '1px solid var(--border)' : 'none',
                }}>
                  <div>
...
function NewClientModal({ onClose, onSave }: { onClose: () => void; onSave: () => void }) {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    await supabase.from('clients').insert({ name: form.name, slug: form.slug.toLowerCase().replace(/\s+/g, '-') })
    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal-box" style={{ maxWidth: '400px' }}>
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div><label className="label">Nome da Campanha *</label><input className="input" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Ex: Campanha João Silva" /></div>
          <div><label className="label">Slug (URL) *</label><input className="input" value={form.slug} onChange={e => setForm(p => ({ ...p, slug: e.target.value.toLowerCase().replace(/\s+/g, '-') }))} placeholder="joao-silva" /></div>
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            <button className="btn-secondary" onClick={onClose}>Cancelar</button>
            <button className="btn-primary" onClick={handleSave} disabled={saving || !form.name || !form.slug}>{saving ? 'Criando...' : 'Criar cliente'}</button>
          </div>
        </div>
      </div>
    </div>
...
function NewUserModal({ onClose, onSave, clients }: { onClose: () => void; onSave: () => void; clients: Client[] }) {
  const supabase = createClient()
  const [saving, setSaving] = useState(false)
  const [form, setForm] = useState({ email: '', full_name: '', password: '', role: 'cliente_visualizacao', client_id: '' })
  ...
  return (
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal-box" style={{ maxWidth: '440px' }}>
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div><label className="label">Nome Completo *</label><input className="input" value={form.full_name} onChange={e => setForm(p => ({ ...p, full_name: e.target.value }))} placeholder="Nome do usuário" /></div>
          ...
          <div><label className="label">Senha provisória *</label><input className="input" type="password" value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} /></div>
          ...
          {error && <div style={{ background: 'var(--red-dim)', border: '1px solid rgba(224,82,82,0.2)', borderRadius: '8px', padding: '10px', color: 'var(--red)', fontSize: '13px' }}>{error}</div>}
          <div style={{ background: 'var(--blue-dim)', border: '1px solid rgba(82,130,224,0.15)', borderRadius: '8px', padding: '10px', fontSize: '12px', color: 'var(--blue)' }}>
            O usuário receberá um e-mail para confirmar a conta. Você também pode criar diretamente pelo painel do Supabase.
          </div>
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            <button className="btn-secondary" onClick={onClose}>Cancelar</button>
            <button className="btn-primary" onClick={handleSave} disabled={saving}>{saving ? 'Criando...' : 'Criar usuário'}</button>
          </div>
        </div>
      </div>
    </div>
